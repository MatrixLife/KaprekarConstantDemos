...
#define CONST_6174 6174

int _generate_random_value()
{
	return 1000 + (int)(rand() / (float)(RAND_MAX) * 8999.0f);
};
...
/**/
	...
	size_t rounds = 0;
/**/
	cprintf("Kaprekar Constant demo (4-digit_chars)\r\n");
	srand(time(NULL));
	if(argc > 1)
	{
		arg_value = atoi(argv[1]);
		if(arg_value >= 1000 && arg_value <= 9999)
		{
			raw_value = arg_value;
